...
可选 Gap 库仅保留 6 条 BASIC L1–L6，均为53 bp，不含制备用GG接头。BSEVA_L1、76/14 bp接口、24 bp占位及26/34 bp抗性接口均不再归库。已拆出的边界以内容哈希留存审计，校验不依赖可选 Gap 库。临时骨架占位改用BASIC L1，生成最终质粒时由完整表达构建替换。新旧请求均只插入用户明确选择的Gap。
