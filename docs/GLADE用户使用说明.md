...
也可以使用网页，自己选择抗性模块和复制模块：

```powershell
python main.py web -i demo01.json
```

浏览器会自动打开。点击组件或从组件库拖入圆环、右侧列表；按住圆环色块或列表行拖动即可调整排列顺序，两处同步更新。选择两个模块，检查结果通过后点击“生成设计”，即可下载按当前顺序拼接的质粒 GenBank、制备片段和报告；结果自动登记到 manifest。程序补齐 T1、固定间隔及两端酶切位点，保留上传元件和当前表达构建。
