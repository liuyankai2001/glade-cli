...
浏览器会自动打开。点击组件或从组件库拖入圆环、右侧列表；按住圆环色块或列表行拖动即可调整排列顺序，两处同步更新。T0、T1 在“终止子”分类中手动加入，参考位置是“ T1 → 完整表达构建 → T0 ”。缺少终止子或位置不正确时，点击“生成设计”会弹出警告，可选择“继续生成”或“返回调整”；继续生成时只使用当前选入的序列。酶切冲突等错误仍需先解决。结果可导出 GenBank、制备片段和报告，并登记到 manifest。程序保留固定间隔、上传元件和当前表达构建，补齐两端酶切位点。
