...
也可以使用网页，自己选择抗性模块、复制模块和终止子：

```powershell
python main.py web -i demo01.json
```

浏览器会自动打开。每次点击抗性组件或终止子，或从组件库拖入圆环、右侧列表，都会在列表末尾新增一个独立副本；同一个组件可以重复添加。每个副本可分别删除、拖动排序，圆环和列表同步更新，刷新页面后保留全部副本和顺序。复制模块保留一个，再次选择时替换原模块。按住圆环色块或列表行拖动即可调整排列顺序。T0、T1 在“终止子”分类中手动加入，参考位置是“ T1 → 完整表达构建 → T0 ”。缺少终止子或位置不正确时，点击“生成设计”会弹出警告，可选择“继续生成”或“返回调整”；继续生成时只使用当前选入的序列。酶切冲突等错误仍需先解决。结果可导出 GenBank、制备片段和报告，并登记到 manifest，所有选入副本都会进入实际序列和导出文件。程序保留固定间隔、上传元件和当前表达构建，补齐两端酶切位点。

网页首版需要一个完整表达构建、大肠杆菌宿主和前面指定的两种常规黏性末端酶。存在额外酶位点时，更换酶或模块后重试；更换酶需要重新完成 CDS 检查和表达构建。输出位于 `plasmid_designs/`。按 Ctrl+C 停止网页服务；端口占用时加 `--port 8001`。
